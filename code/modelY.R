...
A <- cbind(int=1, ppi=dt$ppi.l3, dt[,xvar])
G <- as.matrix(dt[,lg]) 
...
model{
  for (i in 1:n) {
     y[i] ~ dnegbin(p[i],r)
     p[i] <- r/(r+lambda[i]) 
     log(lambda[i]) <- log(pop[sid[i]] - mu*cu[i]) - log(pop[sid[i]]) + A[i,] %*% beta + log(xi + Ghc[i])
  }
...
# estimation 
## list parameters
bayes.mod.params <- c("beta","gamma","xi","r","mu")

### run mcmc
mod.fit <- jags.parallel(data=mydata, parameters.to.save = bayes.mod.params, n.iter=40000, n.chains=3, model.file="jags-model.jag", n.thin=10)
samples<- as.mcmc(mod.fit)
 
saveRDS(samples, file=file.path(psdir,"postsam.ppi2casesY.rds"))
