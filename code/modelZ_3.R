...
tdf.l3 <- within(csts.df, {
  tid <- as.numeric(date - start.date) + 15
  ppi.l3 <- ppi
})[c("state","tid","ppi.l3")]
...
### run mcmc
mod.fit <- jags.parallel(data=mydata, parameters.to.save = bayes.mod.params, n.iter=100000, n.chains=3, model.file="jags-modelY.jag", n.thin=20)
samples<- as.mcmc(mod.fit)

saveRDS(samples, file=file.path(psdir,"postsam.ppi2casesZ_3.rds"))
